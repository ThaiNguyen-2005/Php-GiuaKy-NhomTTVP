...
export default function Home({ setActiveTab }: { setActiveTab: (id: string) => void }) {
  return (
    <div className="p-8 space-y-10">
      {/* Stats Section */}
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-primary-container p-6 rounded-xl flex items-center justify-between scholar-shadow group hover:-translate-y-1 transition-transform cursor-pointer" onClick={() => setActiveTab('my-books')}>
          <div>
        ...
        <div className="bg-tertiary-container p-6 rounded-xl flex items-center justify-between scholar-shadow group hover:-translate-y-1 transition-transform cursor-pointer" onClick={() => setActiveTab('my-books')}>
          <div>
      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setActiveTab('catalog')} className="text-primary font-medium hover:underline flex items-center gap-1">
            Xem tất cả <span className="material-symbols-outlined text-sm">arrow_forward</span>
          </button>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8">
          {newBooks.map(book => (
            <div key={book.id} className="flex flex-col group cursor-pointer" onClick={() => setActiveTab('catalog')}>
